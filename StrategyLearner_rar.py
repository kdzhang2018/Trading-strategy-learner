...
class StrategyLearner(object):
    ...
    def __init__(self, verbose = False, rar = 0.98):
...
#             print daily_rets
        ...
        self.learner = ql.QLearner(num_states=1000, num_actions = 3, alpha = 0.2, \
            gamma = 0.9, rar = self.rar, radr = 0.999, dyna = 0, verbose=False)
        # train the QLearner
        epochs = 200
...
if __name__=="__main__":
    ...
    port_val_df = pd.DataFrame(np.arange(0.01, 1, 0.01), columns = ['rar'])
    port_val_df['port_val'] = np.zeros(port_val_df.shape[0])
    for i in range(port_val_df.shape[0]):
        rar = port_val_df.ix[i, 'rar']
        print "************** rar = ", rar, " *************"
        verb = False
            # instantiate the strategy learner
        learner = StrategyLearner(verbose = verb, rar = rar)
        ...
        port_val_df.ix[i, 'port_val'] = port_val.values[0]/1000000
        
    print port_val_df
    ax = port_val_df.plot(kind = 'scatter', x = 'rar', y = 'port_val', title = "AAPL port_val vs rar")
    fit = np.polyfit(port_val_df['rar'], port_val_df['port_val'], deg=1)
    ax.plot(port_val_df['rar'], fit[0] * port_val_df['rar'] + fit[1], color='red')
