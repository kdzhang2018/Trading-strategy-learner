...
#                     print states.index[day], reward.values[0], action, holding, port_val
    ...
    def testPolicy(self, symbol = "IBM", sd=dt.datetime(2009,1,1), ed=dt.datetime(2010,1,1), sv = 10000):
        ...
        prev_holding = 0
        reward = 0
#         daily_rewards = states.copy() # for making a plot
...
#             #df.plot(kind = 'scatter', x = 'daily_rets', y = 'holdings', title = 'AAPL holding vs daily return')
#             for day in trades.index:
#                 if trades.ix[day, 0] > 0: plt.axvline(x=day, color='g', linestyle='--')
#                 elif trades.ix[day, 0] < 0: plt.axvline(x=day, color='r', linestyle='--')
#             #plt.axvline(x=pd.to_datetime('2009-01-01'), color='r', linestyle='-')
...
if __name__=="__main__":
    ...
    df_trades = learner.testPolicy(symbol = sym, sd = stdate, ed = enddate, sv = 1000000)
